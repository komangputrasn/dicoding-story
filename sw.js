...
self.addEventListener("fetch", (event)=>{
    ...
    if (!event.request.url.startsWith(self.location.origin) && !event.request.url.includes("unpkg.com") && !event.request.url.includes("openstreetmap.org")) return;
...
self.addEventListener("sync", (event)=>{
    ...
    if (event.tag === "background-sync-story") event.waitUntil(syncOfflineStories());
});
...
async function syncOfflineStories() {
    try {
        ...
        for (const story of offlineStories)try {
...
function openDB() {
    return new Promise((resolve, reject)=>{
        ...
        request.onupgradeneeded = (event)=>{
            ...
            if (!db.objectStoreNames.contains("offline-stories")) db.createObjectStore("offline-stories", {
                keyPath: "id",
                autoIncrement: true
            });
        };
